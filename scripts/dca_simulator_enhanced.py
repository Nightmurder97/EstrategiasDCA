...
class EnhancedDCASimulator:
    ...
    def plot_enhanced_analysis(self, market_data: Dict[str, pd.DataFrame]):
        ...
        fig_height = max(15, num_assets * 0.5)  # Aumentar altura según cantidad de activos
        ...
        try:
            ...
            if not market_data or not isinstance(market_data, dict):
                raise ValueError("Datos de mercado no válidos o vacíos")
        
            # Recolectar datos de retornos
            for symbol in self.symbols:
                if symbol in market_data:
                    prices = market_data[symbol]['Close']
                    if len(prices) > 1:  # Asegurar que hay suficientes datos
            ...
            if returns_data.empty:
                plt.text(0.5, 0.5, 'Datos insuficientes para calcular correlaciones',
                        ha='center', va='center', transform=ax5.transAxes)
            else:
                # Validar que hay suficientes datos para la correlación
                if returns_data.shape[1] < 2:
                    plt.text(0.5, 0.5, 'Se necesitan al menos 2 activos para la correlación',
                            ha='center', va='center', transform=ax5.transAxes)
                else:
                    # Calcular y limpiar la matriz de correlación
                    corr_matrix = returns_data.corr().round(2)
                    
                    # Crear máscara para el triángulo superior
                    mask = np.triu(np.ones_like(corr_matrix), k=1)
                    
                    # Configurar el heatmap con mejores parámetros
            ...
            print(f"Error al generar matriz de correlación: {str(e)}")
            plt.text(0.5, 0.5, f'Error al generar matriz de correlación:\n{str(e)}',
                     ha='center', va='center', transform=ax5.transAxes)
